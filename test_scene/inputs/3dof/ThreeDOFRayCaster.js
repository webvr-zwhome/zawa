...
// var VrHeadModel = require('VrHeaderModel');
const LEFT_ORIGIN = [-0.3, 5, -0.3];
const RIGHT_ORIGIN = [0.3, 5, -0.3];
...
export default class ThreeDOFRayCaster extends RayCaster {
  constructor(scene, headModelPosition) {
    super();
    // debugger;
    this._scene = scene;
    this._active = true;
    ...
    // console.log(this._headModelPosition.prototype === headModelPosition.prototype);
    // this._parentObj = new THREE.Object3D();
    ...
    window.addEventListener('gamepaddisconnected', (e) => {
      if (this._gamepadIndex === e.gamepad.index) {
        ...
        this._scene.remove(this._parentObj);
        this._gamepadIndex = -1;
        this._active = false;
      }
    });
  ...
  _setUpGamepad(gamepad) {
    ...
    this._scene.add(this._parentObj);
    // this._scene.add(this._mesh);
    ...
    if (gamepad.hand === 'left') {
      ...
      this._parentObj.position.set(0, 0, 0);
      this._parentObj.quaternion.set(0, 0, 0, 0);
      this._mesh.position.set(LEFT_ORIGIN[0], LEFT_ORIGIN[1], LEFT_ORIGIN[2]);
  ...
  _createController() {
    ...
    this._parentObj = new THREE.Object3D();
    this._parentObj.add(this._mesh);
    console.log(this._parentObj);
    window.mesh = this._mesh;
    // window.mesh = this._parentObj;
  }
  ...
  frame() {
    ...
    // console.log('headModelPosition', this._headModelPosition);
    // console.log('window: ', window.headModelPosition);
    ...
    //     var displayPos = vrDisplay.getPose().position;
    //     // console.log('display: ', displayPos[0]);
    //   }
    // });
    ...
    if (gamepad && gamepad.pose && gamepad.pose.orientation) {
      ...
      if(orientation) {
        const rotation = window.headModelPosition.rotate;
        let rotateVec = new THREE.Vector3(0, 1, 0);
        const sinAngle = Math.sin(rotation);
        // const y = 1 * sinAngle;
        // const w = Math.cos(rotation);
        const y = 0;
        const w = 0;
        this._parentObj.rotation.y = rotation/180*Math.PI;
        this._mesh.quaternion.set(orientation[0], orientation[1], orientation[2], orientation[3]);
        
      }
      if(position) {
        // this._parentObj.position.set(window.headModelPosition.x, window.headModelPosition.y, window.headModelPosition.z);
        this._parentObj.position.set(window.headModelPosition.x, window.headModelPosition.y, window.headModelPosition.z);
        
        // this._mesh.position.set(position[0] + window.headModelPosition.x, position[1] + 5, position[2] + window.headModelPosition.z);
        this._mesh.position.set(position[0] + LEFT_ORIGIN[0], position[1], position[2] + LEFT_ORIGIN[2]);
        console.log('_mesh: ',this._mesh.position);
      }
