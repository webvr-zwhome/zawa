import * as THREE from 'three';

const path = [
    new THREE.Vector3( 50, 12, 0 ),
    new THREE.Vector3( 80, 24, -30 ),
    new THREE.Vector3( 66, 26, -50 ),
    new THREE.Vector3( 46, 30, -75 ),
    new THREE.Vector3( 0, 36, -100),
    
    new THREE.Vector3( -66, 30, -60),
    new THREE.Vector3( -100, 20, -30),
    new THREE.Vector3( -100, 30, 0),
    new THREE.Vector3( -100, 20, 30),
    new THREE.Vector3( -66, 20, 60),

    new THREE.Vector3( 0, 14, 80 ),
    new THREE.Vector3( 0, 14, 0 ),
    new THREE.Vector3( 0, 20, -80 ),
    new THREE.Vector3( -46, 16, -65),
    new THREE.Vector3( -46, 16, -35),

    new THREE.Vector3( -70, 8, 0 ),
    new THREE.Vector3( -46, 8, 30),
    new THREE.Vector3( 0, 20, 60 ),
    new THREE.Vector3( 20, 26, 10 ),
    new THREE.Vector3( 70, 20, 10),

    // new THREE.Vector3( 120, 20, 50 ),
    new THREE.Vector3( 120, 10, 60 ),
    new THREE.Vector3( 100, 10, 100 ),
    new THREE.Vector3( 30, 20, 160 ),
    new THREE.Vector3( -10, 20, 60 ),
    new THREE.Vector3( 0, 20, 0 ),

    new THREE.Vector3( 40, 24, -100 ),
    new THREE.Vector3( 80, 20, -50 ), 
    new THREE.Vector3( 80, 10, 80 ), 
    new THREE.Vector3( 20, 10, 80 ), 
    new THREE.Vector3( 20, 10, 0 ),

    new THREE.Vector3( -20, 30, -100 ),
    new THREE.Vector3( -80, 30, -100 ),
    new THREE.Vector3( -100, 20, 0 ),
    new THREE.Vector3( -100, 20, 100 ),
    new THREE.Vector3( 0, 5, 100 ),

    new THREE.Vector3( 20, 12, 80 ),
    // new THREE.Vector3( 50, 12, 0 ),
]
export {
   path,
}